...
public class DatabaseGenerator {
    ...
    private void processDeptsAndCollections(Map<String, String> row, List<String> deptsAdded, List<String> collectionsAdded) {
        ...
        Dept dept;
        Collection coll;
        if (!deptsAdded.contains(deptName)) {
            dept = new Dept();
            // id will be auto-generated by save(), right? How to get that value?
            dept.setId(deptsAdded.size());
            dept.setName(deptName);
            deptRepo.save(dept);
        }
        // Its dept is guaranteed to exist at this point, so get its id from deptsAdded DICTIONARY
        if (!collectionsAdded.contains(collName)) {
            coll = new Collection();
            // id will be auto-generated, right?
            coll.setName(collName);
            collectionRepo.save(coll);
        }
        return;
    }
